"""Compute SVD of each expert's w1 and cache the full V basis for project-out.

For each expert, w1 has shape [d_ff, hidden_dim].  SVD gives V of shape
...
Files: {cache_dir}/{float|8bit}/{model_tag}_layer{L}_expert{i}.pkl
"""
...
def run_svd_from_expert(
    ...
    """Compute SVD of each expert w1 → save V (columns = vectors) per expert.
    ...
    config = ExperimentConfig(svd_dir="", layer_idx=layer_indices[0],
                              model_id=model_id, model_tag=model_tag,
                              num_experts=num_experts, quantization=quantization)
    ...
    for layer_idx in layer_indices:
        ...
        for expert_idx in range(num_experts):
            w1 = _weight_float(
                model.model.layers[layer_idx].block_sparse_moe.experts[expert_idx].w1,
                quantized,
            ).cpu().float()
            _, _, Vh = torch.linalg.svd(w1, full_matrices=False)
        ...
        logger.info("Layer %d: saved %d expert V matrices to %s", layer_idx, num_experts, out_path)
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Compute SVD of each expert w1; save V for project-out on the router.",
    )
    p.add_argument("--model-id", default="mistralai/Mixtral-8x7B-v0.1")
    p.add_argument("--model-tag", default="mistralai_Mixtral_8x7B_v0.1")
    p.add_argument("--cache-dir", default="svd_cache", help="Base cache dir (subdirs: float/, 8bit/)")
    p.add_argument("--layer_idx", type=int, nargs="+", required=True, help="Layer(s) to export")
    p.add_argument("--num-experts", type=int, default=8)
    p.add_argument("--quantization", default=None, choices=("8bit", "4bit"))
    args = p.parse_args()

    run_svd_from_expert(
        model_id=args.model_id, model_tag=args.model_tag,
        cache_dir=args.cache_dir, layer_indices=args.layer_idx,
        num_experts=args.num_experts, quantization=args.quantization,
    )
